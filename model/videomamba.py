...
class Block(nn.Module, mamba_init):
    ...
    def flatten_spectral_spatial(self, x):
        # x = rearrange(x, 'b c t h w -> b c (h w) t')  # [B,8,128*5,64,64]
        # x = rearrange(x, 'b c n m -> b c (n m)')  # [10, 192, 1792]


        B, C,  T,  H, W = x.shape
        S = int(T/self.TIME_STEPS)
        x = rearrange(x, "b c (t s) h w -> b c (t h w s)", t=self.TIME_STEPS, s=S, h=H, w=W)
        return x
    ...
    def reshape_spectral_spatial(self, y, B, H, W, T):
        # y = y.transpose(dim0=1, dim1=2).contiguous() # (1, 8, H * W * T)
        # y = y.view(B, H * W, T, -1)  # (1, H * W * T, 8)>>> (1, H * W, T, 8)
        # y = rearrange(y, 'b o t c -> b t o c')  # (1, T, H * W, 8)
        # y = y.view(B, T, H, W, -1)  # (1, T, H,  W, 8)


        y = rearrange(y, "b c (t h w s) -> b c (t s) h w", t=self.TIME_STEPS, h=H, w=W, s=int(T/self.TIME_STEPS))
    ...
    def forward(self, x: Tensor, SelectiveScan = SelectiveScanMamba):
        ...
        out_y = out_y.view(B, K, -1, L)  #[10, 384, 64]->[10, 2, 192, 64]    [10, 4, 192, 64]
...
class VisionMamba(nn.Module):
    def __init__(
        ...
        self.conv3d_features1 = nn.Sequential(
            nn.Conv3d(conv3D_channel, out_channels=1, kernel_size=conv3D_kernel),
            # nn.BatchNorm3d(1),
            # nn.ReLU(),
        )

        self.embedding_spatial_spectral = nn.Sequential(nn.Linear(conv3D_channel, embed_dim))  # [64, 28, 8, 8, 32]
    ...
    def forward_features(self, x, inference_params=None):
        x = self.conv3d_features(x)  #[10, 1, 30, 15, 15]->[10, 32, 28, 8, 8]  [B,T,C,H,W]
        #scan
        ...
        x = self.conv3d_features1(x.permute(0, 4, 1, 2, 3))
        return x


    def forward(self, x, inference_params=None):
        B, C, T, H, W = x.shape
        latent = x.permute(0, 2, 1, 3, 4).contiguous()
        latent = latent.view(B, -1, H, W)  # (4,128,5,64,64)==>
        latent = latent.unsqueeze(1)
        feature = self.forward_features(latent, inference_params)  ##[10, 192]
        latent = feature.squeeze(1).view(B, T, C, H, W)
